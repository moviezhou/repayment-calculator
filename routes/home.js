...
var dic = new InterestDict();



exports.home = function(req,res){
	
	res.render('home',{title: 'HKPow'})
};

exports.input_handler = function(req, res){
	var contract_id = req.body.id;
	var contract_time = req.body.date;
	var contract_money = req.body.amount;
	var contract_duration = req.body.year;

	var borrower = new Borrower(contract_id,contract_money,contract_time,contract_duration);
    var rc = new RepaymentCalculator(dic,borrower);
    rc.calc_repay_seq();

    var data = [];

    for(var item in rc.repayment_detail_seq){
    	data.push({
    		serial_num: rc.repayment_detail_seq[item].serial_num,
    		m_principal: rc.repayment_detail_seq[item].m_principal,
    		m_interest: rc.repayment_detail_seq[item].m_interest,
    		m_PrinInter: rc.repayment_detail_seq[item].m_PrinInter,
    		principal_balance: rc.repayment_detail_seq[item].principal_balance
    	});
    }

    data.push({
        sum_principal_paid: rc.sum_principal_paid,
        sum_interset_paid: rc.sum_interset_paid
    });
